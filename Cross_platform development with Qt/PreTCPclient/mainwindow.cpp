...
{
    ui->tb_result->append("Текущая дата: " + time.date().toString("dd.MM.yyyy") + " Время: " + time.toString("hh:mm"));
}
void MainWindow::DisplayFreeSpace(uint32_t freeSpace)
{
    ui->tb_result->append("Свобоное место: ");
    ui->tb_result->append(QString::number(freeSpace));
}
...
void MainWindow::DisplayStat(StatServer stat)
{
    ...
    ui->tb_result->append("Получено байт: " + QString::number(stat.incBytes));
    ...
    ui->tb_result->append("Клиенты: " + QString::number(stat.clients));
    ui->tb_result->append("Время работы: " + QString::number(stat.workTime));
}
void MainWindow::DisplayError(uint16_t error)
{
    switch (error) {
    case ERR_NO_FREE_SPACE:{
         ui->tb_result->append("Ошибка! Нет свободного места");
         break;
    }
    case ERR_NO_FUNCT:{
         ui->tb_result->append("Ошибка! Несуществующая функция");
         break;
    }
    default:
...
void MainWindow::DisplaySuccess(uint16_t typeMess)
{
    switch (typeMess) {
    case CLEAR_DATA:{
         ui->tb_result->append("Данные очищены");
        break;
    }
    default:
         ui->tb_result->append("Операция выполнена");
        break;
    }
