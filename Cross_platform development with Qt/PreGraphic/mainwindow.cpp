    ...
    , ui(new Ui::MainWindow)
{
    ...
    chart->setTitle("График найденных результатов за секунду");
    ...
    connect(this, &MainWindow::sig_ready_to_draw, this, &MainWindow::draw_graph);
...
void MainWindow::on_pb_start_clicked()
{
    ...
    auto findMax = [&](QVector<double> res){
                                                ...
                                                data = res.mid(0, qMin(res.size(), 1000));
...
void MainWindow::draw_graph(){
    ...
    if (graphWindow == nullptr) {
            graphWindow = new QMainWindow();
            graphWindow->setCentralWidget(chartView);
            graphWindow->resize(600, 400);
            graphWindow->setWindowTitle("График");
        }



    chart->axes(Qt::Horizontal).first()->setRange(0, 1000);
    double minY = *std::min_element(data.begin(), data.end());
    double maxY = *std::max_element(data.begin(), data.end());
    chart->axes(Qt::Vertical).first()->setRange(minY - 0.01, maxY + 0.01);
